...
const useLireMorph = () => {
  ...
  const selectedRadical = useMemo(() => {
    ...
    const chosenRadical = radicals[selectedRadicalIndex];
    setIsPrefixesDisable(chosenRadical.prefixes.length === 0);
    setIsSuffixesDisable(chosenRadical.suffixes.length === 0);
