...
const useLireMorph = () => {
  ...
  const updateAffixesBasedOnFilters = useCallback((prefixes: PrefixesEnum[], suffixes: SuffixEnum[]): void => {
    data.setAffixes(prevAffixes => {
      ...
      const existingAffixes = data._filterAffixesThatWillBeVisible(prevAffixes, targetAffixes);
      const newAffixTexts = data._findAffixesThatNeedToBeAdded(existingAffixes, targetAffixes);
      
      const usedPositions = existingAffixes.map(affix => affix.position);
      const availablePositions = data.shuffleCells(data.cellsGrid).filter(pos => 
      ...
      const newAffixes = newAffixTexts.map((text, i) => ({
        text,
        position: availablePositions[i] || data.shuffleCells(data.cellsGrid)[i],
      }));
  ...
  }, [data.showPrefixes, data.showSuffixes, data.cellsGrid, data.affixes.length, _cleaningFittedAffixes]);
  ...
  const selectedRadical = useMemo(() => {
    if (data.selectedRadicalIndex === null) return lireMorphText.example.radical;
  ...
  const moveAffix = (e: MouseEvent) => {
    ...
    const leftPercent = Math.min(Math.max(((e.clientX - containerRect.current.left - dragState.current.offset.x) / containerRect.current.width) * 100, PADDING), 100 - PADDING);
    const topPercent = Math.min(Math.max(((e.clientY - containerRect.current.top - dragState.current.offset.y) / containerRect.current.height) * 100, PADDING), 100 - PADDING);
    ...
    data.setAffixes((prev) => {
      ...
      newAffixes[indexToUpdate] = { ...newAffixes[indexToUpdate], position: { x: leftPercent, y: topPercent } };
      return newAffixes;
    });
  };
  ...
  const fitWithRadical = () => {
    ...
    const isLeftOccupied = Object.entries(data.fittedAffixes).some(([idx, side]) => parseInt(idx) !== dragState.current.currentIndex && side === 'left');
    const isRightOccupied = Object.entries(data.fittedAffixes).some(([idx, side]) => parseInt(idx) !== dragState.current.currentIndex && side === 'right');
    ...
    if (fitPosition.x !== 0 && fitPosition.y !== 0 && fittedSide) {
      data.setFittedAffixes(prev => ({ ...prev, [dragState.current.currentIndex!]: fittedSide }));
  ...
  useEffect(() => {
    const _update = () => {
      if (data.selectedRadicalIndex === null) {
        updateAffixesBasedOnFilters(lireMorphText.example.prefixes, lireMorphText.example.suffixes);
      } else {
        updateAffixesBasedOnFilters(radicals[data.selectedRadicalIndex].prefixes, radicals[data.selectedRadicalIndex].suffixes);
      }
    };

    const radicalChanged = previousRadicalIndex.current !== data.selectedRadicalIndex;
    const filtersChanged = previousShowPrefixes.current !== data.showPrefixes || 
    ...
    previousShowSuffixes.current = data.showSuffixes;
  }, [data.selectedRadicalIndex, data.showPrefixes, data.showSuffixes, updateAffixesBasedOnFilters, data.setFittedAffixes]);
